---
title: 'Simulations of Urban Microclimate in ANSYS Fluent'
date: 2016-09-19
permalink: /posts/2016/09/blog-post-1-urban-microclimate/
tags:
  - Urban Microclimate
  - ANSYS Fluent
...
![urban](http://jialeishen.github.io/images/urban.png)

A brief introduction on urban microclimate and its simulations using ANSYS Fluent.
...
## 1. <a name="1-introduction-绪论"></a>Introduction 绪论 
When using ANSYS Fluent® for urban microclimate simulation, it commonly requires users to apply some user defined functions (UDFs) to simulate the realistic urban environment and further solve specific urban microclimate problems, such as pollutant transmission, in ANSYS Fluent. This article introduced some commonly used UDF files of ANSYS Fluent for urban microclimate simulations, including the UDFs of wind velocity profile (inlet profile), pollutant emission source, tree and greening, and some commonly used performance indices for evaluating the urban ventilation. Some relevant publications are presented at the end of this article. This article and associated codes were originally posted on [my Github](https://github.com/jialeishen/UDF-of-Urban-Microclimate).

在使用商用CFD软件ANSYS Fluent®研究城市微气候（Urban Microclimate）时，常常需要根据城市微气候的各特点编写用户自定义函数（UDF, i.e. User Defined Function）来模拟真实的城市环境，本文介绍了其中一些常用的UDF。本文首先对UDF及城市微气候的基本概念各自进行了简单的介绍；并结合城市微气候中的概念对本文档中的UDF进行了简单介绍，目前本文档中的UDF主要针对城市风环境；最后推荐了一些城市微气候相关的书籍及论文文献。希望本文档中所讨论的内容能对各位同行今后的科研有所帮助。你可以在[我的Github](https://github.com/jialeishen/UDF-of-Urban-Microclimate)上找到本文及相关的UDF文件。

## 2. <a name="2-introduction-of-udf-udf简介"></a>Introduction of UDF UDF简介 
There have been a bunch of documents talking about UDFs. This article only focuses on UDFs related to urban microclimate. The background knowledge regarding UDF syntax and C programming is not discussed in this document. You can find numerous information online (for example, from [the ANSYS Fluent Help file for UDFs](https://www.afs.enea.it/project/neptunius/docs/fluent/html/udf/node4.htm#:~:text=A%20user%2Ddefined%20function%2C%20or,standard%20features%20of%20the%20code.)). UDFs are written in C language. Therefore, users need to have some fundamental background in C programming or at least programming, no matter what language it is. The codes of UDFs need to satisfy both C syntax and UDF "rules" (using predefined functions and macros in Fluent, check the Help file for more). UDFs in Fluent can be used to define specific properties for different variables, including boundary profiles (e.g. with varying velocity, temperature, concentration , etc.), material properties, chemical reaction constant, species source/sink term, and user defined scalars (UDSs), etc. 
...
## 3. <a name="3-introduction-of-urban-microclimate-城市微气候简介"></a>Introduction of Urban Microclimate 城市微气候简介 
...
Urban microclimate mainly include **Urban Thermal Environment, Urban Moist Environment, Urban Wind Environment (Urban Ventilation), Urban Pollutant and Atmospheric Environment, Urban Lighting Environment, and Urban Sound Environment**, etc. This article only focuses on **Urban Wind Environment (Urban Ventilation)** and **Urban Pollutant and Atmospheric Environment** as those were my major research areas. 

城市微气候主要包括：**城市热环境、城市湿环境、城市风环境、城市污染与大气环境、城市光环境、城市声环境**等。由于本人的研究方向及个人水平的限制，目前本文所涉及到的城市微气候内容只局限在对**城市风环境**与**城市污染与大气环境**的讨论。
...
![urbanphysics](http://jialeishen.github.io/images/urbanphysics.png)

## 4. <a name="4-UDFs-in-Urban-Microclimate-城市微气候相关UDF"></a>UDFs in Urban Microclimate 城市微气候相关UDF 
...
![inletflow](http://jialeishen.github.io/images/inletflow.png)
...
## 5. <a name="5-Tutorials-of-Application-of-UDFs-in-Fluent-相关UDF解释与教程"></a>Tutorials of Application of UDFs in Fluent 相关UDF解释与教程 

Now you have a brief overview on different UDFs of urban microclimate. But how can you really use them in Fluent and what does each command/code of the UDF file mean? You can always look up the Help file of Fluent and it can likely solve almost 100% of all questions you have. I have also drafted a few tutorials documents about that, which are listed below and also posted on this site. Feel free to find and read them if you needed. It might provide you some helps on understanding UDF files and the applications of them. All these documents were written in Chinese. For English-speaking readers, you can actually find many similar tutorials or documents on Google or Youtube. Please do so if you need. 

UDF相关的代码与各类宏的介绍，可以参见ANSYS Fluent官方的UDF教程，你可以在你的Fluent的Help中找到相关文档。网络上亦有一些相关的教程，请自行搜索。本文中涉及到的一些UDF代码均带有一些注释，可以配合注释来理解这些UDF代码。此外，我也以部分UDF文件为例，写了比较详尽的解释文档，如下所示，你也可以在我的其他贴子中找到它们。这应该会对你的理解有所帮助。这些文档写于多年前，如有任何错误请大家指出。
...
## 6. <a name="6-More-Information-更多阅读"></a>More Information 更多阅读 
...
限于篇幅，本文档对于城市微气候的介绍十分有限，如果有兴趣继续了解更多城市微气候相关的内容，以下是一些我个人推荐的书籍与综述性论文，供各位参考学习：
...
Feel free to let me know if you have any questions or comments on this document or my researches. Corrections or suggestions on the contents in this document are always welcome. I'm open to have any discussions with you. You can always find me through this site.

本文介绍了几个常用的城市微气候的UDF，同时简单介绍了城市微气候相关内容，希望这些UDF和介绍内容可以对各位同行日后的科研有所帮助或启发，同时**强烈邀请**大家对本文内容进行修改及补充！！也欢迎同行分享更多城市微气候相关的UDF，欢迎共同交流。

*Originally posted on [my Github](https://github.com/jialeishen/UDF-of-Urban-Microclimate) on 2016/09/19*

*Recently updated on 2022/04/30*

*Posted on this site on 2022/05/07*
